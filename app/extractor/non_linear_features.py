...
def _compute_rpde(signal: np.ndarray,m: int = 4,tau: int = 1,T_max: int = 200,target_sr: int = 8000,original_sr: int = 22050) -> float:
    try:
...
def _compute_dfa(signal: np.ndarray, n_scales: int = 16) -> float:
    try:
        n = len(signal)
...
def _compute_d2(signal: np.ndarray, m: int = 4, tau: int = 1) -> float:
    try:
...
def _compute_ppe(log_period: np.ndarray, bins: int = 50) -> float:
    try:
        if len(log_period) < 2:
            return np.nan
